...
module Vents
  ...
  def neighbours(floor, row, col)
    [[row - 1, col],
     ...
     [row, col - 1],
     [row, col + 1]]
      .filter { |wrow, wcol| (0...floor.length).cover?(wrow) && (0...floor.first.length).cover?(wcol) }
  end
  ...
  def bfs(floor, start)
    ...
    until frontier.empty?
      ...
      neighbours(floor, current_row, current_col)
        .filter do |row, col|
          !visited.include?([row, col]) && floor[row][col] >= floor[current_row][current_col] && floor[row][col] < 9
        end
  ...
  def load(filename)
    File
      .readlines(filename)
      .map { |row| row.strip.chars.map(&:to_i) }
  end

  def low_points(floor)
    (0...floor.length)
      .to_a.product((0...floor.first.length).to_a)
      .filter do |(row, col)|
